...
import type { MarketFeeParams, UserFeeRates } from '@/contexts/builderFee/ports/types';
...
export interface UseUserFeesResult {
   ...
   *
   * @param marketParams - Optional market parameters for HIP-3 fee calculation
   * @returns Promise resolving to user fee rates, or undefined if failed
   *
   * @example
   * ```tsx
   * // For validator perps (e.g., BTC)
   * useEffect(() => {
   *   loadUserFees();
   * }, [loadUserFees]);
   *
   * // For HIP-3 markets with growth mode
   * useEffect(() => {
   *   loadUserFees({
   *     isHip3: true,
   ...
   * ```
   */
  loadUserFees: (marketParams?: MarketFeeParams) => Promise<UserFeeRates | undefined>;
}
...
export function useUserFees(): UseUserFeesResult {
   ...
   * @param marketParams - Optional market parameters for HIP-3 fee calculation
   */
  const loadUserFees = useCallback(
    async (marketParams?: MarketFeeParams): Promise<UserFeeRates | undefined> => {
      if (!wallet) {
      ...
      try {
        const rates = await getUserFeesUseCase.execute({
